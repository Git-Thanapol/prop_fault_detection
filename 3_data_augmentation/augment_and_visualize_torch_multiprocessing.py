...
class AudioAugmentor:
    def __init__(self):
        ...
        if os.path.exists(RIR_PATH) and len(os.listdir(RIR_PATH)) > 0:
            try:
                ...
                pass # Silently fail in worker to avoid log spam if path invalid
    ...
    def apply_waveform_augmentation(self, y, aug_type):
        ...
        elif aug_type == "reverse": return np.flip(y_aug).copy()
    ...
    def apply_spectral_masking(self, spec_tensor, aug_type):
        if aug_type == "time_masking":
            ...
            # Check dimensions if possible or use safe defaults
            if spec_tensor.shape[-2] == N_MELS: param = 20
            else: param = 100
...
def init_worker():
    """Initialize Torch/Audiomentations objects once per worker."""
    global _augmentor
...
def process_file_wrapper(filepath):
    global _augmentor
    if _augmentor is None: _augmentor = AudioAugmentor()
